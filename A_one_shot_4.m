...
% modified: label, when same class->0, different classes-> 1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear;
...
for i = 1:size(test_data, 2)
    ...
    %w = dlarray(single(test_data(1:8, i+160)), 'CT');
    %v = dlarray(single(test_data(9:16, i+160)), 'CT');
    %dlY(i+5) = predict_one_shot(dlnet,fcParams,w,v);
    %Y(i+5) = gather(extractdata(dlY(i+5)));
    %Y(i) = round(Y(i));
end
