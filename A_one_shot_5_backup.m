...
function A_one_shot_5_backup(input)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% modified: "get_pair_label" function error
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% testing
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[test_data, test_label] = load_test_data(str_test);  % load test data
disp(length(test_data));
...
for i = 1:sz
   ...
   data(1:8, i+sz) = reshape(n(1:4,1:2,i+2*sz), [8,1]);
   data(9:16, i+sz) = reshape(n(1:4,1:2,i+3*sz), [8,1]);
   label(i+sz,1) = 0;
end
end
...
pre_data = y(:, 1:3);
test_time = y(:, 4:9);
test_data(:, 1:2) = mapminmax(pre_data(:, 1:2)')';
test_data(:, 3) = pre_data(:, 3);
[test_data, test_time] = sliding_mean(test_data, test_time);
[p, n] = generate_p_n(test_data);
if length(p) == 1 && p == 0
    [test_data, test_label] = get_pair_label(n(:,:,1:30), n(:,:,31:size(n, 3)));
else
